...
//-------------------------------------------
    public UsuarioVO getDatosUsuEdit(Integer id_usu) {
        ...
        String q = "SELECT id_usu, nombre_usu,nombre2_usu," +
                   ...
                   "email,area,anexo,curriculum,foto  FROM usuario,area" +
                   " WHERE usuario.id_area=area.id_area " +
                   "AND usuario.id_usu=?";
        try {
            conn = ConnectionPool.getInstance().getConnection();
            ...
            if (rs.next()) {
                ...
                usuario.setFoto(rs.getBytes(14));
...
*/
        try {
            ...
            String q = "UPDATE usuario SET curriculum=?" +" WHERE id_usu=?";
            PreparedStatement ps = conn.prepareCall(q);
            FileInputStream in = new FileInputStream(curriculo.getCurriculumFile());
            ps.setBinaryStream(1, in, (int)curriculo.getCurriculumFile().length()); 
            ...
            FileInputStream instream = new FileInputStream(curriculo.getCurriculumFile());
...
//            File file= a.byteToFile(b);
    ...
    public boolean actualizarFoto(byte[] foto, Integer idUsu) {
        ...
        try {
            ...
            conn = ConnectionPool.getInstance().getConnection();
            //conn.setAutoCommit(false);
            //String q = "UPDATE usuario SET foto=empty_blob()" +" WHERE id_usu=?";
            String q = "UPDATE usuario SET foto=?" +" WHERE id_usu=?";
            PreparedStatement ps = conn.prepareCall(q);
            FileInputStream in = new FileInputStream(fileFoto);
            ps.setBinaryStream(1, in, (int)fileFoto.length()); 
            ps.setInt(2, idUsu.intValue());
            ps.executeUpdate();
           // conn.commit();
