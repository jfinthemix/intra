...
public class InfoInstitDAO {
    ...
    public boolean upArchivo(ArchivoInfoVO arch) throws Exception {
        ...
        String q = "INSERT INTO informacioninstitucional (id_info_instit,descripcion,nombre_archivo,archivo)" +
                   " values (s_informacioninsitucional.nextval,'" +
                   arch.getDescripcion() + "','" +
        ...
        try {
            ...
            q =
                    "SELECT archivo FROM informacioninstitucional WHERE id_info_instit=" +
                    nuevo;
            ps = conn.prepareCall(q);
            rs = ps.executeQuery();
            rs.next();
    ...
    /**
     * listaArchivos
     * entrega una lista de los archivos disponibles
     *
     * @return List
     */
    public List listaArchivos() {
        ...
        String que =
                "SELECT id_info_instit,nombre_archivo,descripcion FROM informacioninstitucional";
        try {
            conn = ConnectionPool.getInstance().getConnection();
    ...
    public ArchivoDocVO getArchivoDoc(Integer idArchivo) {
        Connection conn;
        String q =
                "SELECT archivo,nombre_archivo FROM informacioninstitucional  WHERE id_info_instit=?";
    ...
    public boolean eliminarArchivo(Integer idArchivo) throws Exception {
        try {
            Connection conn;
            String q =
                    "Delete informacioninstitucional  WHERE id_info_instit=?";
            ArchivoDocVO a = new ArchivoDocVO();
            conn = ConnectionPool.getInstance().getConnection();
            PreparedStatement ps = conn.prepareCall(q);
            ps.setInt(1, idArchivo.intValue());
