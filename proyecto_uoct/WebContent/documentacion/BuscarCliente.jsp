...
%>
<html>
<head><jsp:include page="../comunes/head.jsp" />
...
<div class="col-md-6 searchtab tabMini">
				 				<form id="form1" action="clienteAction.do">
				 				 <input type="hidden" name="hacia" value="buscarCli"/>
				 				 <input type="hidden" name="PorPalabra" value="PorPalabra"/>
  									<div class="form-group">
  										<label for="inputBusca1">
  											<span class="glyphicons glyphicons-search"></span><br>
  											Por palabra clave<br>
  										</label>
    									<input type="text" name="palClave" class="form-control" id="inputBusca1">
  									</div>
  									<a OnClick="submitThisForm1();" class="botoVerde busca" name="PorPalabra" href="javascript:void(0)">Buscar</a>
								</form>
				 			</div>
...
<div class="col-md-6 searchtab tabMini">
				 				...
				 				<input type="hidden" name="Particulares" value="Particulares"/>
  									<div class="form-group">
  										<label>
  									...
  									<a OnClick="submitThisForm2();" class="botoVerde busca" name="Particulares" href="javascript:void(0)">Ver</a>
								</form>
				 			</div>
</div>
...
<display:table name="icl"    requestURI="clienteAction.do" id="icls" class="table table-striped table-bordered table-hover tablesorter" >
        <display:caption>Lista de Clientes </display:caption>

        <display:column sortable="true" media="html" sortProperty="nomCli" title="Cliente" paramId="id_cli" paramProperty="idCli">
        <a href="javascript:Llamadalink('detalleCliente','<%=((CliEntVO)icls).getIdCli()%>','<%=((CliEntVO)icls).getNomCli()+" "+((CliEntVO)icls).getApeCli()%>')"><%=((CliEntVO)icls).getNomCli()+" "+((CliEntVO)icls).getApeCli() %></a> 
        </display:column>

        <display:column  media="excel pdf"  title="Cliente">
        <%=((CliEntVO)icls).getNomCli()+" "+((CliEntVO)icls).getApeCli() %> </display:column>

        <display:column media="html" sortable="true" title="Entidad externa" href="javascript:Llamadalink('detalleEntExt','#')" paramId="id_ent" paramProperty="idEnt">
        ...
        <display:setProperty name="paging.banner.last" value="	<span class=\"pagelinks\">[<a href=\"{1}\">Primero</a>/<a href=\"{2}\">Pr</a>                <a href=\"{2}\">ev</a>]{0}[Sgte/Ultimo]"></display:setProperty>
...
function Llamadalink(hacia,link,nom){
	...
	if (window.opener) {
		...
		link=link.replace('#','');
		link='documentacion/clienteAction.do?id_cli='+link+'&hacia='+hacia;
		//alert(link+' ' +param);
		LlamadaPagina(link);
	}
