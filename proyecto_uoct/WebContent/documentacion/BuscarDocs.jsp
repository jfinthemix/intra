...
function SeleccionaOpcion(opcion){
	...
	if(opcion==1){
		...
		$('#divSentido').css("display", "none");
					
	}else{
		
		$('#divCodigo').css("display", "none");
		$('#divSentido').css("display", "block");
	}
...
<form action="documentoAction.do" id="formFormulario" name="buscarDocs" class="form-horizontal">
<input type="hidden" id="hidTipoBus" name="tipoBus" value=""/>
<input type="hidden" name="hacia" value="busDoc"/>
...
<div class="form-group"  id="divSentido">
    								<label for="inputSentido" class="col-sm-4 control-label">Sentido</label>
    								<div class="col-sm-8">
    								
    								<input type="hidden" id="hidSentido" name="id_sentido" value=""/>
      								<label class="radio-inline">
      										
  											<input name="id_sent" type="radio" name="opcionesSentido" onClick="seleccionSentido()" id="sent1" value="1"> Entrante
										</label>
										<label class="radio-inline">
  											<input name="id_sent" type="radio" name="opcionesSentido" onClick="seleccionSentido()" id="sent2" value="2"> Saliente
										</label>
    								</div>
    							</div>
<div class="form-group">
    ...
    <div class="form-group input-daterange">
    								...
    								<div class="col-sm-3">
      								<input name="fecha_ini" type="date" class="form-control inputFecha" id="inputDesde">
    								</div>
    								<div class="col-sm-1">
  			...
  			<div class="boxOpciones">
    								<div class="form-group">
    									<div class="col-sm-12">
      									...
      									<a name="gobuscardoc" href="javascript:submitThisForm()" class="botoVerde busca"><span class="glyphicons glyphicons-search"></span> Buscar</a>
      								</div>
  									</div>
    							</div>
 ...
 <h4>Documentos encontrados</h4>
        ...
        <display:table class="its"  name="docses"  requestURI="documentoAction.do"  id="docss">
        <display:setProperty name="export.amount" value="list"/>

          <display:caption>Documentos Encontrados</display:caption>

        <display:column title="Código" media="html" sortable="true"  paramId="id_doc" paramProperty="idDoc">
		<a href="javascript:Llamadalink('detalleDoc','<%=((DocumentodeListaVO)docss).getIdDoc()%>','<%=((DocumentodeListaVO)docss).getTipoDoc()%>','<%=((DocumentodeListaVO)docss).getCodDoc()%>')" ><%=((DocumentodeListaVO)docss).getTipoDoc()+"-"+((DocumentodeListaVO)docss).getCodDoc()%></a> 
        </display:column>
		...
		<display:column title="Materia" ><%=((DocumentodeListaVO)docss).getMateriaDoc()%> </display:column>
		<display:column title="Fecha" maxLength="14"  ><%=sdf.format(((DocumentodeListaVO)docss).getFechaio())%></display:column>
...
if (window.opener) {
	...
	var formulario= $('#formFormulario');
	var action='documentacion/documentoAction.do';
	
	var tipo= $('#hidTipoBus').val();
	...
	if(tipo==1){
		formulario.validate({
			  rules: {
			    ...
			    messages: {
			    	id_tipoDoc: {
				    	required:"Debe seleccionar un código de documento."
	...
	{ 
		formulario.validate({
			  rules: {
			    ...
			    messages: {
			    	id_sent: {
				    	required:"Debe seleccionar un sentido."
...
function Llamadalink(hacia,link,tipo,cod){
	
	if (window.opener) {
		    
		pasaDoc(link,cod,tipo);
				   
		   
		}else{
		link=link.replace('#','');
		link='documentacion/documentoAction.do?id_doc='+link+'&hacia='+hacia;
		//alert(link+' ' +param);
		LlamadaPagina(link);
	}
...
function seleccionSentido()
{
 	var sentido=$('input[name=id_sent]:checked', '#formFormulario').val();
	$('#hidSentido').val(sentido);
