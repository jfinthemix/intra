...
import proyecto_uoct.usuario.controller.UsuarioFormBean;
...
//OBTIENE LA LISTA DE DOCUMENTOS COINCIDENTES CON LOS PARAMETROS(ACTIVOS E INACTIVOS)
    ...
    private ActionForward ingresarDocumento(ActionMapping actionMapping,
        ...
        try {
            ...
            if(eldoc!=null){
	            File doc1 = new File(eldoc.getFileName());
	            DataOutputStream dostr = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(doc1)));
	            dostr.write(eldoc.getFileData());
	            dostr.close();
	            newDoc.setfFile1(doc1);
            }
            if(eldoc1!=null){
	            File doc2=new File(eldoc1.getFileName());
	            DataOutputStream dostr2 = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(doc2)));
	            dostr2.write(eldoc1.getFileData());
	            dostr2.close();
	            newDoc.setfFile2(doc2);
            }
            if(eldoc2!=null){
	            File doc3=new File(eldoc2.getFileName());
	            DataOutputStream dostr3 = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(doc3)));
	            dostr3.write(eldoc2.getFileData());
	            dostr3.close();
	            newDoc.setfFile3(doc3);
            }
            ...
            if (documentoLocal.insertarDocumento(newDoc)) {
                servletRequest.setAttribute("mensaje","El documento fue ingresado exitosamente");
                return actionMapping.findForward("mensaje_fw");
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private ActionForward goBusDoccod(ActionMapping actionMapping,
        ...
        try {
            ...
            busdocs.setIdTipoDoc(Integer.valueOf(servletRequest.getParameter("id_tipoDoc")));
            busdocs.setCodigoDoc(servletRequest.getParameter("codDoc"));
            busdocs.setMateria(servletRequest.getParameter("materia"));
            busdocs.setFechaIni(servletRequest.getParameter("fecha_ini"));
            busdocs.setFechaFin(servletRequest.getParameter("fecha_fin"));
    ...
    private ActionForward goBusDocSen(ActionMapping actionMapping,
        ...
        try {
            ...
            BusDocsVO busdocs = new BusDocsVO();
            busdocs.setIdSentido(Integer.valueOf(servletRequest.getParameter("id_sentido")));
            busdocs.setMateria(servletRequest.getParameter("materia"));
            busdocs.setFechaIni(servletRequest.getParameter("fecha_ini"));
            busdocs.setFechaFin(servletRequest.getParameter("fecha_fin"));
            busdocs.setPalClave(servletRequest.getParameter("palClave"));

            busdocs.setEncargado(Integer.valueOf(servletRequest.getParameter("id_usuario")));
